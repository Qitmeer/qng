...
func (sp *Spectre) votedPast(virtualBlock IBlock) *MeerDAG {
...
func (sp *Spectre) updateVotes(votedPast *MeerDAG, vh hash.Hash) bool {
...
func (sp *Spectre) updateTipVotes(voter *SpectreBlock, maxParent *SpectreBlock, votedPast *MeerDAG) {
...
func (sp *Spectre) voteFromFutureSet(votedPast *MeerDAG) *HashSet {
	tips := NewHashSet()
...
func (sp *Spectre) voteFromPastSet(votedPast *MeerDAG, tips *HashSet) bool {
	unvisited := util.NewIterativeQueue()
...
func (sp *Spectre) newVoter(vh hash.Hash, votedPast *MeerDAG) IBlock {
	sb := SpectreBlockData{hash: vh}
	sb.parents = []*hash.Hash{}
	vhChildren := sp.bd.getBlock(&vh).GetChildren()
