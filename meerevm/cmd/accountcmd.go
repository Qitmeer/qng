...
func accountCreate(ctx *cli.Context) error {
	cfg, err := meer.MakeMeerethConfig(config.Cfg.DataDir)
	if err != nil {
		return err
	}
	// Load config file.
	if file := ctx.String(meer.ConfigFileFlag.Name); file != "" {
		if err := meer.LoadConfig(file, cfg); err != nil {
			utils.Fatalf("%v", err)
		}
	}
...
func accountUpdate(ctx *cli.Context) error {
	...
	for _, addr := range ctx.Args().Slice() {
		account, oldPassword := meer.UnlockAccount(ks, addr, 0, nil)
