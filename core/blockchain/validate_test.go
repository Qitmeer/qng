...
func checkTransactionSanityForAllNet(tx *types.Transaction) error {
	err := CheckTransactionSanity(types.NewTx(tx), &params.TestNetParams, nil, nil)
	if err != nil {
		return err
	}

	err = CheckTransactionSanity(types.NewTx(tx), &params.PrivNetParams, nil, nil)
	if err != nil {
		return err
	}

	err = CheckTransactionSanity(types.NewTx(tx), &params.MixNetParams, nil, nil)
	if err != nil {
		return err
	}

	err = CheckTransactionSanity(types.NewTx(tx), &params.MainNetParams, nil, nil)
	if err != nil {
		return err
	}
	return nil
...
func TestTokenStateRoot(t *testing.T) {
	...
	stateRoot := bc.CalculateTokenStateRoot([]*types.Tx{types.NewTx(createTx())})
...
func TestStateRoot(t *testing.T) {
	bc := BlockChain{}
	expected := "5b7d48b6c505d90b21355081cf4f5a332a925ac87e24ceedd3ddf02e0f387cc3"
	stateRoot := bc.CalculateStateRoot([]*types.Tx{types.NewTx(createTx())})
	if stateRoot.String() != expected {
		t.Fatalf("state root is %s, but expected is %s", stateRoot, expected)
	}
}
