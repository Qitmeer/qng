...
import (
	...
	ophttp "github.com/Qitmeer/qng/rollups/node/http"
	"github.com/Qitmeer/qng/rollups/service/metrics"
...
func NewMetrics(procName string) *Metrics {
	...
	return &Metrics{
		...
		RPCClientRequestsTotal: factory.NewCounterVec(prometheus.CounterOpts{
			...
			Name:      "requests_total",
			Help:      "Total RPC requests initiated by the node's RPC client",
		}, []string{
			"method",
		}),
		...
		RPCClientResponsesTotal: factory.NewCounterVec(prometheus.CounterOpts{
			...
			Help:      "Total RPC request responses received by the node's RPC client",
		}, []string{
			"method",
			"error",
		}),
...
// call to the node's RPC server. It bumps the requests metric,
