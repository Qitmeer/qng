...
var (
	...
	VerifierL1Confs = &cli.Uint64Flag{
		...
		EnvVars:   prefixEnvVar("VERIFIER_L1_CONFS"),
		Required: false,
		Value:    0,
	}
...
func CheckRequired(ctx *cli.Context) error {
	l1NodeAddr := ctx.String(L1NodeAddr.Name)
	if l1NodeAddr == "" {
		return fmt.Errorf("flag %s is required", L1NodeAddr.Name)
	}
	l2EngineAddr := ctx.String(L2EngineAddr.Name)
	if l2EngineAddr == "" {
	...
	rpcListenAddr := ctx.String(RPCListenAddr.Name)
	if rpcListenAddr == "" {
		return fmt.Errorf("flag %s is required", RPCListenAddr.Name)
	}
	if !ctx.IsSet(RPCListenPort.Name) {
