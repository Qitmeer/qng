...
func (rate QitPerByte) ToMeerPerKb() MeerPerKilobyte {
	...
	if rate == QitPerByte(-1.0) {
		return -1.0
	}

	return MeerPerKilobyte(float64(rate) * bytePerKb * meerPerQit)
}
...
func NewQitPerByte(fee types.Amount, size uint32) QitPerByte {
	return QitPerByte(float64(fee.Value) / float64(size))
}
...
type FeeEstimator struct {
	...
	mtx      sync.RWMutex
	observed map[hash.Hash]*observedTransaction
...
func NewFeeEstimator(maxRollback, minRegisteredBlocks uint32) *FeeEstimator {
	...
	return &FeeEstimator{
		...
		observed:            make(map[hash.Hash]*observedTransaction),
...
func (ef *FeeEstimator) ObserveTransaction(t *TxDesc) {
	...
	if _, ok := ef.observed[hash]; !ok {
		size := uint32(types.GetTransactionWeight(t.Tx))
...
func (ef *FeeEstimator) RegisterBlock(block *types.SerializedBlock) error {
	...
	transactions := make(map[*types.Tx]struct{})
...
func (b *estimateFeeSet) estimateFee(confirmations int) QitPerByte {
	if confirmations <= 0 {
		return QitPerByte(math.Inf(1))
	}
...
func (ef *FeeEstimator) newEstimateFeeSet() *estimateFeeSet {
	...
	set.feeRate = make([]QitPerByte, capacity)
...
func (ef *FeeEstimator) estimates() []QitPerByte {
	set := ef.newEstimateFeeSet()

	estimates := make([]QitPerByte, estimateFeeDepth)
...
func (ef *FeeEstimator) EstimateFee(numBlocks uint32) (MeerPerKilobyte, error) {
	...
	return ef.cached[int(numBlocks)-1].ToMeerPerKb(), nil
}
...
func RestoreFeeEstimator(data FeeEstimatorState) (*FeeEstimator, error) {
	...
	log.Info("Restore fee estimator")
